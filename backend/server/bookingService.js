...
async function createBooking({ userId, tripId, seats, totalPriceSatang, holdTokens }) {
  ...
  try {
    ...
    const bookingId = bookingIns.rows[0].booking_id;
    ...
    return {
      bookingId,
      status: bookingIns.rows[0].status,
    };
  } catch (e) {
...
async function getBookingDetail(bookingId) {
  const b = await pool.query(
    `SELECT booking_id, user_id, trip_id, total_price, status, booking_date, hold_tokens_json
     FROM booking
     WHERE booking_id = $1`,
    [bookingId]
  );
